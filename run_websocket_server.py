#!/usr/bin/env python
"""
WebSocket server launcher script for resilience testing.

This script starts a ProgressServer instance for use in resilience testing.
...
async def main(args):
    ...
    # Note: message_rate_limit is not supported in the current server implementation
    ...
    try:
        # Start server
        server = await start_websocket_server(**server_config)
        logger.info(f"WebSocket server started on {args.host}:{args.port}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run the WebSocket progress reporting server")
    ...
    # Note: message-rate-limit is not used but kept for backward compatibility
    parser.add_argument("--message-rate-limit", type=int, default=100, help="Message rate limit per second (not currently used)")
